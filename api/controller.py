...
class RestaurantListResource(Resource):
    def get(self,):
        ...
        if 'postalCode' in request.args:
            arg = request.args.get('postalCode')
            addresses = Address.find_by_postal_code(arg)
            restaurants = []
            for address in addresses:
                restaurant = Restaurant.find_by_id(address.restaurant_id)
                restaurants.append(restaurant)
        if 'streetAddress' in request.args:
            arg = request.args.get('streetAddress')
            addresses = Address.find_by_street_address(arg)
            restaurants = []
            for address in addresses:
                restaurant = Restaurant.find_by_id(address.restaurant_id)
                restaurants.append(restaurant)
        if 'addressLocality' in request.args:
            arg = request.args.get('addressLocality')
            addresses = Address.find_by_address_locality(arg)
            restaurants = []
            for address in addresses:
                restaurant = Restaurant.find_by_id(address.restaurant_id)
                restaurants.append(restaurant)
        if 'addressRegion' in request.args:
            arg = request.args.get('addressRegion')
            addresses = Address.find_by_address_region(arg)
            restaurants = []
            for address in addresses:
                restaurant = Restaurant.find_by_id(address.restaurant_id)
                restaurants.append(restaurant)
        if 'addressCountry' in request.args:
            arg = request.args.get('addressCountry')
            addresses = Address.find_by_address_country(arg)
            restaurants = []
            for address in addresses:
                restaurant = Restaurant.find_by_id(address.restaurant_id)
                restaurants.append(restaurant)
